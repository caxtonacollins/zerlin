...
export class PollingService {
  ...
  async handleNetworkStatus() {
    ...
    try {
      ...
      let currentFee = 0;
      try {
        currentFee = await this.stacksService.estimateTransferFee(
          BigInt(1),
          'SP1P72Z3704VMT3DMHPP2CB8TGQWGDBHD8PR1618C',
        );
      } catch (e) {
      ...
      this.logger.debug(
        `Network status updated: Block ${status.blockHeight}, Fee: ${currentFee}`,
      );

      // Update on-chain fee oracle
      try {
        await this.feeOracleService.updateFeeRate(
          currentFee,
  ...
  private async checkAlerts(currentFee: number) {
    ...
    for (const alert of activeAlerts) {
      ...
      if (triggered) {
        this.logger.log(
          `[NOTIFICATION TRIGGERED] Alert ${alert.id} for User ${alert.user.stacksAddress}: Current Fee ${currentFee} is ${alert.condition} ${alert.targetFee}`,
        );
      }
    }
  }
